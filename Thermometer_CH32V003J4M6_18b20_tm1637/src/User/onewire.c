 ...
 *      mateusz@msalamon.pl
 *
 * Modified: Direct register access for pin direction switching
 *           to eliminate dependency on CPU frequency.
 *           GPIO_Init() replaced with direct CFGLR writes.
 */
...
//    Using direct register access instead of GPIO_Init()
//    to avoid frequency-dependent timing overhead.
//    Works correctly at any CPU frequency (24, 48 MHz, etc.)
//
void OneWire_BusInputDirection(OneWire_t *onewire)
{
    uint8_t pin_num = __builtin_ctz(onewire->GPIO_Pin); // get pin number from mask
    onewire->GPIOx->CFGLR &= ~(0xF << (4 * pin_num));  // clear 4 bits for this pin
    onewire->GPIOx->CFGLR |=  (0x4 << (4 * pin_num));  // Input floating (0100)
}
...
void OneWire_BusOutputDirection(OneWire_t *onewire)
{
    uint8_t pin_num = __builtin_ctz(onewire->GPIO_Pin); // get pin number from mask
    onewire->GPIOx->CFGLR &= ~(0xF << (4 * pin_num));  // clear 4 bits for this pin
    onewire->GPIOx->CFGLR |=  (0x3 << (4 * pin_num));  // Output PP 50MHz (0011)
}
...
{
    GPIO_WriteBit(onewire->GPIOx, onewire->GPIO_Pin, 0);
    //onewire->GPIOx->BCR = onewire->GPIO_Pin<<16; // Reset the 1-Wire pin
}


void OneWire_OutputHigh(OneWire_t *onewire)
{
    GPIO_WriteBit(onewire->GPIOx, onewire->GPIO_Pin, 1);
    //onewire->GPIOx->BSHR = onewire->GPIO_Pin; // Set the 1-Wire pin
}
...
uint8_t OneWire_Reset(OneWire_t* onewire)
{
    ...
    i = GPIO_ReadInputDataBit(onewire->GPIOx, onewire->GPIO_Pin); // Check if bus is low
...
uint8_t OneWire_ReadBit(OneWire_t* onewire)
{
    ...
    if (GPIO_ReadInputDataBit(onewire->GPIOx, onewire->GPIO_Pin)) // Read the bus state
        bit = 1;
...
//
//    Select a device by pointer to ROM address
//
