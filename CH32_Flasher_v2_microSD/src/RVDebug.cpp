...
//------------------------------------------------------------------------------
// Standalone-appliance addition: bounded wait, instead of the original
// while(...){} loops that assumed an interactive session and just waited
// forever for a target that might never respond.

static const uint32_t COMM_TIMEOUT_US = 200000; // 200ms - generous for ~800kbps SWIO

// Returns true if `cond` became true within the timeout, false on timeout.
template <typename F>
static bool wait_for(F cond) {
  uint32_t start = time_us_32();
  while (!cond()) {
    if (time_us_32() - start > COMM_TIMEOUT_US) return false;
  }
  return true;
}
...
bool RVDebug::halt() {
  ...
  if (!wait_for([this]{ return get_dmstatus().ALLHALTED != 0; })) {
    LOG("RVDebug::halt() - timed out, no response from target\n");
    comm_ok = false;
    return false;
  }
...
bool RVDebug::reset() {
  ...
  set_dmcontrol(0x80000001);
  if (!wait_for([this]{ return get_dmstatus().ALLHALTED != 0; })) {
    comm_ok = false; return false;
  }

  // Set reset request
  set_dmcontrol(0x80000003);
  if (!wait_for([this]{ return get_dmstatus().ALLHAVERESET != 0; })) {
    comm_ok = false; return false;
  }
...
void RVDebug::run_prog(bool wait_until_not_busy) {
  ...
  if (wait_until_not_busy) {
    if (!wait_for([this]{ return !get_abstractcs().BUSY; })) {
      comm_ok = false;
    }
  }
  else {
