// ch32_flasher_demo
//
// Автономный флешер CH32V003 на плате YD-RP2040.
// 4 кнопки (1.bin / 2.bin / 3.bin / backup-restore), встроенный WS2812
// показывает статус.
// Прошивки читаются с microSD-карты (1.bin / 2.bin / 3.bin), лог - в log.txt
// на той же карте (дублируется во внутреннюю flash Pico как запасной канал,
// если карты вдруг нет).
//
...
//                      отвечает, ИЛИ карта/файл недоступны)
...
// долгое (>=1.5с) = записать backup.bin обратно на чип (та же индикация,
// что у обычной записи 1/2/3.bin).
//
// Резервный (flash) лог автоматически выгружается по UART при каждом
// включении питания / нажатии reset на Pico - ничего отдельно нажимать не
// нужно, просто держи UART-переходник подключенным, когда хочешь посмотреть.
//
// Based on the SWIO/RVDebug/WCHFlash modules from the PicoRVD project
// (https://github.com/Community-PIO-CH32V/PicoRVD, MIT license), and the
// FatFs_SPI library from carlk3/no-OS-FatFS-SD-SPI-RPi-Pico (Apache 2.0).
//
...
Neopixel led1; // GP23
Neopixel led2; // GP22
FlashLog flog; // резервный лог во внутренней flash Pico (работает без SD)
...
static bool sd_mount() {
  ...
  if (fr != FR_OK) {
    printf_r("SD f_mount error: %s (%d)\n", FRESULT_str(fr), fr);
    return false;
  }
  printf_g("// SD card mounted OK\n");
  return true;
}

// Принудительно переподключается к карте перед каждой операцией - если
// карту физически вынимали/меняли, библиотека сама этого не замечает без
// явного переразмонтирования (похожая история была с SWIO - там тоже
// требовалась полная переинициализация перед каждой операцией, не только
// один раз при старте).
static void sd_ensure_mounted() {
  sd_card_t *pSD = sd_get_by_num(0);
  f_mount(NULL, pSD->pcName, 0); // размонтировать текущее состояние
  FRESULT fr = f_mount(&pSD->fatfs, pSD->pcName, 1); // и смонтировать заново
  sd_mounted = (fr == FR_OK);
  if (!sd_mounted) {
    printf_r("SD remount error: %s (%d)\n", FRESULT_str(fr), fr);
  }
}

// Читает файл целиком в fw_buf. true - если получилось прочитать хоть что-то.
static bool sd_load_firmware(const char* filename, uint32_t* out_len) {
  *out_len = 0;
  if (!sd_mounted) {
    printf_r("SD not mounted - can't read %s\n", filename);
    return false;
  }
  FIL fil;
  FRESULT fr = f_open(&fil, filename, FA_READ);
  if (fr != FR_OK) {
    printf_r("f_open(%s) error: %s (%d)\n", filename, FRESULT_str(fr), fr);
    return false;
  }
  UINT br = 0;
...
// Дописывает строку в log.txt на SD (если карта смонтирована). Без разрыва
// работы устройства, если SD нет - просто молча ничего не делает.
static void sd_log_append(const char* text) {
...
// Сохраняет буфер fw_buf[0..len) в файл на SD (перезаписывая, если уже есть).
static bool sd_save_firmware(const char* filename, uint32_t len) {
  if (!sd_mounted) {
    printf_r("SD not mounted - can't save %s\n", filename);
    return false;
  }
  FIL fil;
  FRESULT fr = f_open(&fil, filename, FA_CREATE_ALWAYS | FA_WRITE);
  if (fr != FR_OK) {
    printf_r("f_open(%s, write) error: %s (%d)\n", filename, FRESULT_str(fr), fr);
    return false;
  }
  UINT bw = 0;
...
// Общий помощник: пишет итог операции в оба лога (SD, если есть, и всегда -
// резервный во flash Pico).
...
static FlashResult flash_target(PicoSWIO* swio, RVDebug* rvd, WCHFlash* flash,
  ...
  sd_ensure_mounted();
  if (!sd_load_firmware(filename, &len)) {
    res = FLR_NO_FILE;
  }
...
static FlashResult backup_target(PicoSWIO* swio, RVDebug* rvd, WCHFlash* flash) {
  ...
  if (!sd_mounted) {
    printf_r("SD not mounted - can't save backup\n");
    uint32_t elapsed_ms = (time_us_32() - t0) / 1000;
    if (elapsed_ms < MIN_BUSY_MS) sleep_ms(MIN_BUSY_MS - elapsed_ms);
  ...
  if (res == FLR_OK) {
    ...
    if (!rvd->comm_ok) {
      res = FLR_WRITE_FAILED;
    } else if (!sd_save_firmware("backup.bin", ch32v003_flash_size)) {
...
int main() {
  ...
  flog.load(); // резервный лог во flash - подтягиваем историю из прошлых включений

  show(COL_LOG); sleep_ms(500); show(COL_IDLE); // короткая белая вспышка - сигнал,
                                                 // что сейчас пойдёт дамп лога в UART
  flog.dump_uart(); // и сразу выгружаем по UART - не нужно нажимать ничего отдельно,
                     // просто включил питание (или нажал reset на Pico) с уже
                     // подключенным UART-переходником - лог сам появится в терминале

  printf_g("\n\n// ch32_flasher_demo starting (SD: 1.bin/2.bin/3.bin + erase)\n");
  ...
  printf_g("// Ready. B1(GP16)=1.bin  B2(GP17)=2.bin  B3(GP18)=3.bin  B4(GP19)=erase\n");
