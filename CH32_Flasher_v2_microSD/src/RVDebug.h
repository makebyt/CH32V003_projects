...
struct RVDebug {
  ...
  // Set to false whenever a wait-loop below times out (no response from the
  // target - not connected, not powered, bad wiring, etc). Standalone-appliance
  // addition on top of the original PicoRVD code, which assumed an interactive
  // GDB session and just waited forever.
  bool comm_ok = true;
